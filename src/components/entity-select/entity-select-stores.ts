...
const {config, state} = createState(withProps<EntitySelectGlobalState>({
...
export const EntitySelectTypeStore = (type: EntityType): Store<{state: EntityTypeState,config: undefined, name: 'entity-select-type-store'}> => {
  ...
  const {state, config} = createState(withProps<EntityTypeState>({
    filter: null,
    selectedGroup: null,
